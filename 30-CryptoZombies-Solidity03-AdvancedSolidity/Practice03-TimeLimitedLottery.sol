...
import "./Practice03-Lottery.sol";
import "./Practice03-Ownable.sol";

// A lottery that does not allow placing bet consecutively until a certain period of time.
contract TimeLimitedLottery is Lottery, Ownable {
    ...
    */
    ...
    /*
     * View the addresses that are ready for betting; returns an array and the number of items returned
    */
    // !!! This can only be called by the contract creator
    function checkBetReadyAddr() onlyOwner public view returns(address[] memory, uint){
        address[] memory readyAddresses = new address[](addrList.length);

        uint counter = 0;
        for (uint i = 0; i < addrList.length; i++) {
            address loopedAddr = addrList[i];
            if (isReadytoBet(loopedAddr)) {
              readyAddresses[counter] = loopedAddr;
              counter++;
            } 
        }
        return (readyAddresses, counter);
    }
}